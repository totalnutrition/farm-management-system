...
export default async function OrganizationCatalogsPage() {
  ...
  const [breeds, diagnoses, routes, cull, repro, vax, treatments, feed, vet] = await Promise.all([
    ...
    loadCatalog(admin, "org_repro_protocols", "name", "slug, name, description, protocol_type, is_seed", orgIdOrAny),
    loadCatalog(admin, "org_vaccination_protocols", "name", "slug, name, target_class, description, is_seed", orgIdOrAny),
    loadCatalog(admin, "org_treatment_protocols", "name", "slug, name, diagnosis_code, description, is_seed", orgIdOrAny),
    loadCatalog(admin, "org_feed_materials", "name", "name, category, dm_pct, ne_l_mcal_per_kg, cp_pct, ndf_pct, starch_pct, is_seed", orgIdOrAny),
    loadCatalog(admin, "org_vet_medicines", "name", "name, brand, active_ingredient, category, route, default_dose, withdrawal_milk_hours, withdrawal_meat_days, is_seed", orgIdOrAny),
  ]);
    ...
    <div className="flex flex-col gap-6 py-4">
      <header className="flex flex-col gap-1">
        ...
        <p className="text-xs text-muted-foreground">
          Tenant-wide reference data. Seed rows ship with the product; you
          can add your own at any time. Locations inherit from these.
        </p>
      </header>

      <Section title="Reproduction protocols" count={repro.length} hint="Synchronization, resync, and fixed-time-AI protocols.">
        <CatalogTable cols={["Name", "Type", "Description"]}
          rows={repro.map((r) => [
            <SeedCell key={r.slug as string} name={r.name as string} isSeed={r.is_seed as boolean} />,
            (r.protocol_type as string) ?? "—",
            (r.description as string | null) ?? "—",
          ])} />
      </Section>

      <Section title="Vaccination protocols" count={vax.length} hint="Per-class vaccination schedules.">
        <CatalogTable cols={["Name", "Target class", "Description"]}
          rows={vax.map((v) => [
            <SeedCell key={v.slug as string} name={v.name as string} isSeed={v.is_seed as boolean} />,
            v.target_class as string,
            (v.description as string | null) ?? "—",
          ])} />
      </Section>

      <Section title="Treatment protocols" count={treatments.length} hint="Drug + dose + route + withdrawal for common diagnoses.">
        <CatalogTable cols={["Name", "Diagnosis", "Description"]}
          rows={treatments.map((t) => [
            <SeedCell key={t.slug as string} name={t.name as string} isSeed={t.is_seed as boolean} />,
            (t.diagnosis_code as string | null) ?? "—",
            (t.description as string | null) ?? "—",
          ])} />
      </Section>

      <Section title="Feed materials" count={feed.length} hint="Forages, grains, byproducts, minerals. Powers TMR recipes and feeding events.">
        <CatalogTable cols={["Name", "Category", "DM %", "NEL Mcal/kg", "CP %", "NDF %", "Starch %"]}
          rows={feed.map((f) => [
            <SeedCell key={f.name as string} name={f.name as string} isSeed={f.is_seed as boolean} />,
      ...
      <Section title="Veterinary medicines" count={vet.length} hint="Drug catalog with default dose, route, and withdrawal periods.">
        <CatalogTable cols={["Name", "Brand", "Active", "Category", "Route", "Default dose", "Milk WD (h)", "Meat WD (d)"]}
          rows={vet.map((v) => [
            <SeedCell key={v.name as string} name={v.name as string} isSeed={v.is_seed as boolean} />,
          ...
          {rows.length === 0 ? (
            <tr>
              <td colSpan={cols.length} className="px-3 py-3 text-center text-muted-foreground">
                Empty. Catalog tables may not be migrated yet — apply
                migration 0019.
              </td>
            </tr>
          ) : (
